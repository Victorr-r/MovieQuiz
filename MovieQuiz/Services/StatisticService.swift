...
extension StatisticService: StatisticServiceProtocol {
	...
	var totalAccuracy: Double {
		...
		return Double(totalCorrect) / Double(totalAsked) * 100.0
	}
	
	func store(correct count: Int, total amount: Int) {
		let currentTotalCorrect = storage.integer(forKey: Keys.totalCorrectAnswers.rawValue)
		let currentTotalAsked = storage.integer(forKey: Keys.totalQuestionsAsked.rawValue)
		
		storage.set(currentTotalCorrect + count, forKey: Keys.totalCorrectAnswers.rawValue)
		storage.set(currentTotalAsked + amount, forKey: Keys.totalQuestionsAsked.rawValue)
		gamesCount += 1
